...
Deep protocol docs for the four Athenaeum skills (design, reconcile, ratify, audit). Agents should read the relevant section only when executing that mode, not during skill selection.
...
## §10 Dialectic vocabulary

The vocabulary that names the moves of grilling. Greek for the soul of the inquiry, Latin for the form on the page.

### The synthesis

**An unexamined claim does not exist.** Until the claimant applies measurement-machinery (verifier, source, confidence rating), there is nothing for the observer to measure. The producer-side discipline is what gives a claim enough specificity to be evaluated.

| Frame | What it says |
|---|---|
| Observer principle (consumer) | Until a downstream reader independently verifies, the claim is provisional |
| Unexamined-claim (producer) | Until the claimant grounds it, there's nothing to measure |
...
| sed contra | The strongest opposing argument | One designated counter with highest authority |
...
Use these as prefixes on grill-log entries:
...
- **Not a substitute for verifiers.** A formally-disputed normative claim is still a value judgment; a formally-disputed empirical claim should also have a runnable verifier.
